...
class SearchMagnet(Handler):
    ...
    def get(self):
        ...
        output = query.get_magnets(output)
...
def config(today):
    ...
    if config is None:
        ...
        memcache.set('config'+today, config)
    return config
...
def set_star_ratings(releases):
    ...
    for movie in r_dict['movies']:
        if movie['ratings']['critics_score'] != -1 and movie['ratings']['audience_score'] != -1:
            movie['star_rating'] = ((movie['ratings']['critics_score'] + movie['ratings'][
                'audience_score']) / 2.0) / 20.0
...
def add_trackers_to_magnets(magnet_list):
    for item in magnet_list:
        item += configuration.TRACKERS
    return magnet_list


app = webapp2.WSGIApplication([('/', MainPage),
                               ('/search/(.+)', SearchResults),
                               ('/search_magnet.json', SearchMagnet)], debug=False)
